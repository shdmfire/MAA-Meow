...
import com.aliothmoon.maameow.controller.maa.engine.core.AsstApiCallback
...
/**
 * MAA Core AIDL 服务实现。
 * 封装 [MaaCoreLibrary] 的 JNA 调用，通过 Binder 暴露给 app 进程。
 *
 * 继承 [MaaCoreService.Stub] 使此类成为 AIDL Binder 实现，
 * 其生命周期与远程进程一致。
 */
class MaaCoreServiceImpl(private val ctx: MaaCoreLibrary?) : MaaCoreService.Stub() {
    ...
    private val callback = AtomicReference<MaaCoreCallback?>(null)
    ...
    override fun DestroyInstance() {
        val handle = instance.get()
        if (handle != null && ctx != null) {
            Ln.i("$TAG: DestroyInstance()")
            ctx.AsstDestroy(handle)
        }
        instance.set(null)
    }

    override fun hasInstance(): Boolean = instance.get() != null
    ...
    override fun AsyncConnect(adbPath: String?, address: String?, config: String?, block: Boolean): Int {
        val core = requireMaaCore() ?: return 0
        val handle = requireHandle() ?: return 0
        return core.AsstAsyncConnect(handle, adbPath, address, config, if (block) 1.toByte() else 0.toByte()).also {
    ...
    override fun GetTasksList(): IntArray {
        ...
        return if (count > 0) buffer.copyOf(count.toInt()) else IntArray(0)
    }

    override fun Start(): Boolean {
        val core = requireMaaCore() ?: return false
        val handle = requireHandle() ?: return false
        return core.AsstStart(handle).also { Ln.i("$TAG: Start() = $it") }
    }

    override fun Stop(): Boolean {
        val core = requireMaaCore() ?: return false
        val handle = requireHandle() ?: return false
        return core.AsstStop(handle).also { Ln.i("$TAG: Stop() = $it") }
    }
    ...
    override fun BackToHome(): Boolean {
        val core = requireMaaCore() ?: return false
        val handle = requireHandle() ?: return false
        return core.AsstBackToHome(handle).also { Ln.i("$TAG: BackToHome() = $it") }
    }

    override fun AsyncClick(x: Int, y: Int, block: Boolean): Int {
        val core = requireMaaCore() ?: return 0
        val handle = requireHandle() ?: return 0
        return core.AsstAsyncClick(handle, x, y, if (block) 1.toByte() else 0.toByte())
    }

    override fun AsyncScreencap(block: Boolean): Int {
        val core = requireMaaCore() ?: return 0
        val handle = requireHandle() ?: return 0
        return core.AsstAsyncScreencap(handle, if (block) 1.toByte() else 0.toByte())
    }

    override fun GetImage(): ParcelFileDescriptor? {
        val core = requireMaaCore() ?: return null
        val handle = requireHandle() ?: return null
        return getImageInternal { buffer, size -> core.AsstGetImage(handle, buffer, size) }
    }

    override fun GetImageBgr(): ParcelFileDescriptor? {
        val core = requireMaaCore() ?: return null
        val handle = requireHandle() ?: return null
        return getImageInternal { buffer, size -> core.AsstGetImageBgr(handle, buffer, size) }
    }

    private fun getImageInternal(getImageFunc: (Pointer, Long) -> Long): ParcelFileDescriptor? {
        return try {
            ...
            memoryFile.writeBytes(buffer.getByteArray(0, actualSize.toInt()), 0, 0, actualSize.toInt())
            val fd = getFileDescriptor(memoryFile)
            if (fd != null) ParcelFileDescriptor.dup(fd) else { memoryFile.close(); null }
        } catch (e: Exception) {
    ...
    override fun GetVersion(): String = ctx?.AsstGetVersion() ?: "MaaCore not loaded"
    ...
    override fun GetUUID(): String {
        ...
        return if (size > 0) buffer.getString(0) else ""
    }

    private fun requireMaaCore(): MaaCoreLibrary? = ctx ?: run { Ln.e("$TAG: MaaCore not loaded"); null }
    private fun requireHandle(): Pointer? = instance.get() ?: run { Ln.e("$TAG: No instance created"); null }
}
